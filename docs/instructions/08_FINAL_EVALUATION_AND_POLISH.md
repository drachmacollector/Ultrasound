...
- **Output:** Modified `src/realtime/app.py`, `src/realtime/pipeline.py`, `scripts/render_annotated_video.py`, `app_streamlit.py`, `app_gradio.py`. Updated `demo1_walkthrough.md`.
...
- `app_streamlit.py`'s own docstring states it is "Primary web interface (Streamlit). See app_gradio.py for the Gradio backup," but `docs/phases/demo1/demo1_walkthrough.md` (dated 2026-08-26, the most detailed Demo 1 doc) describes the architecture exclusively around `app_gradio.py` and never mentions Streamlit as primary. **[MANUAL]** Confirm with the project owner which is actually intended as primary for the hospital demo (Streamlit's docstring is the more recent-sounding signal, given it explicitly calls Gradio "the backup," but this should be a explicit decision, not an inference) — then update `demo1_walkthrough.md` §1–3 to reflect that decision consistently, rather than leaving both docs asserting different things.
...
- `assets/fetscan_uiux_fix_brief.md` exists with an unclear completion/actioned status. **[MANUAL]** Read it and determine: is this a completed-and-shipped brief, a still-open task list, or an abandoned draft? Resolve by either (a) marking it done and cross-referencing what implemented it, (b) folding any still-open items into this Phase 8 plan's Task 6.4 (UI polish pass) if genuinely still relevant and small, or (c) archiving it with a one-line note if superseded. Do not leave it in its current ambiguous state.
- **Output:** Updated `demo1_walkthrough.md`, updated `09_MASTER_CHECKLIST.md`, resolved status for `fetscan_uiux_fix_brief.md`.
...
- `README.md` (repo root, 8KB) should be audited and updated as the actual front door for anyone (hospital IT staff, a department head, a technical evaluator) landing on the repository. **[MANUAL]** Read the current README fully first.
- Ensure it accurately reflects, in plain, non-jargon-heavy language up top: what the system does, its explicit non-goals/limitations (no clinical validation, research datasets only — matching `00_PROJECT_OVERVIEW.md` §8 verbatim in spirit), how to run the primary demo (once Task 6.1 resolves which app that is), and a short "results at a glance" section (headline macro-F1, cross-device honest gap, NatalIA-derived numbers once Stage 2 completes) linking out to `docs/EVAL_REPORT.md` for full detail rather than duplicating it.
- **Output:** Updated `README.md`.
- **Acceptance criterion:** A person with no prior context can read the README top-to-bottom and correctly answer "what is this, how do I run the demo, and what are its known limitations" without opening any other file.
