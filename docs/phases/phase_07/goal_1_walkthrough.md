...
| File | Change |
|---|---|
| `src/models/multitask_model.py` | Architecture; eval-mode anchor split fix |
| `scripts/build_multitask_manifest.py` | Patient-level split via filename prefix + hashlib.md5; FP/MULTICENTRE excluded; -100 cls labels |
| `scripts/verify_no_multitask_leakage.py` | Non-vacuous Check 2 (patient-ID coverage >= 95%); Check 3 scoped to annotated rows |
| `scripts/train_multitask.py` | -100 masking in metrics; val NaN guard; .detach().item() loss accumulation |
