...
## 2. Cross-device generalization (UCL/HC18 held-out set)

- Same metrics as above, computed **separately**, restricted to the 3 classes this dataset covers (head/abdomen/femur — map our brain sub-planes to a single "head" bucket if the UCL dataset doesn't distinguish sub-planes, and document that mapping explicitly).
- Report the **gap** between in-distribution and cross-device performance per class — this gap *is* the generalization result, more informative than either number alone. Expect a meaningful drop; the UCL paper's own experiments found exactly this pattern and used it to argue for multi-centre training. If our drop is severe, that's a legitimate, reportable finding, not a failure to hide — document it plainly.
...
- **Mean latency-to-stabilize after a genuine plane change** (using the manually-marked transition points from `05_TEMPORAL_SMOOTHING_AND_REALTIME.md` §A2) — measures responsiveness.
...
Produce a single `EVAL_REPORT.md` (or notebook rendered to markdown) at the end covering all of the above, with the confusion matrix and class-distribution charts embedded as images. This is the artifact that documents "does this thing actually work and by how much," separate from the code itself.

## Deliverables checklist

- [ ] In-distribution test metrics computed and reported
- [ ] Cross-device generalization metrics computed and reported, gap explicitly called out
...
- [ ] `EVAL_REPORT.md` written, including honest statement of limitations (video-transition validation gap, UCL set only covering 3/7 classes, no true clinical validation)
