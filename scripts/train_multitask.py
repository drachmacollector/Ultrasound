...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(loader):
        if len(batch) == 4:
            images, labels, bboxes_list, class_labels_list = batch
        else:
            images, labels = batch
            bboxes_list = None
...
def train(cfg: dict) -> None:  # type: ignore[type-arg]
    ...
    # Use bboxes_path from config, default to data/processed/bboxes.json
    bboxes_path = cfg.get("bboxes_path", "data/processed/bboxes.json")
    
    train_dataset = FocalPlanesDataset(
        csv_path=cfg["train_csv"],
        transform=get_train_transform(img_size=img_size, mean=normalize_mean, std=normalize_std, with_bboxes=True),
        bboxes_path=bboxes_path
    )
    val_dataset = FocalPlanesDataset(
        csv_path=cfg["val_csv"],
        transform=get_eval_transform(img_size=img_size, mean=normalize_mean, std=normalize_std, with_bboxes=True),
        bboxes_path=bboxes_path
    )
